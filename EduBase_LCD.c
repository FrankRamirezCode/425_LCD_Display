...
void EduBase_LCD_Ports_Init(void)
{
    ...
    // Enable the digital functionality for the PC6 pin
    ...
    // Configure the PE0 pin as output
    // by setting Bits 0 in the DIR register
    GPIOA->DIR |= 0x01;

    // Configure the PE0 pins to function as
    // GPIO pins by clearing Bits 0 in the AFSEL register
    GPIOA->AFSEL &= ~0x01;

    // Enable the digital functionality for the PE0 pins
    // by setting Bits 0 in the DEN register
    GPIOA->DEN |= 0x01;

    // Initialize the output of the PE0 pins to zero
    // by clearing Bits 0 in the DATA register
    GPIOA->DATA &= ~0x01;
}
...
void EduBase_LCD_Diplay_Heart(void)
{
	EduBase_LCD_Enable_Display();
	EduBase_LCD_Clear_Display();
